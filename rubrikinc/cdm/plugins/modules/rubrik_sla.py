...
DOCUMENTATION = '''
module: rubrik_sla
short_description: Create or update an SLA Domain.
description:
    - Create or update SLA Domain.
version_added: '2.8'
...
options:
  ...
  replication_target:
    description:
      - Name of cluster already added to the source cluster to use as a replication target.
    required: false
    default: None
  replication_retention_in_days:
    description:
      - Number of days to retain backups on replication target.
    required: false
    default: None
  timeout:
    description:
...
- rubrik_sla:
    ...
    replication_retention_in_days: 10
'''
...
respofull_response_cdm_5:
    description: The full API response for POST /v2/sla_domain or PATCH /v2/sla_domain?id=<ID>.
    returned: on success when connected to a CDM v5.0 or greater cluster
    type: dict

idempotent_response:
    description: A "No changed required" message when the Rubrik SLA is already present with the identical configuration on the cluster.
...
def main():
    ...
    replication_target = ansible["replication_target"]
    replication_retention_in_days = ansible["replication_retention_in_days"]
    ...
    try:
        ...
        if "configuration does not match" in str(error):
            try:
